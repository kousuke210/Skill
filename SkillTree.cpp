...
void SkillTree::setup_nodes() {
    ...
    // ノードデータ (push_backに修正して古いC++環境に対応)
    nodes_.clear(); // 既存のノードをクリア

    // ID 0: ルートスキル
    nodes_.push_back(SkillNode{ 0, -1, VIEWPORT_W / 2 - NODE_SIZE / 2, NODE_SPACE_Y * 1, "スタート", h_icon, 0, "ツリーの起点スキル。", true });

    // ティア 2
    nodes_.push_back(SkillNode{ 1, 0, VIEWPORT_W / 2 - NODE_SIZE / 2 - NODE_SPACE_X, NODE_SPACE_Y * 3, "攻撃力アップ I", h_icon, 10, "物理攻撃力が少し上昇します。", false });
    nodes_.push_back(SkillNode{ 2, 0, VIEWPORT_W / 2 - NODE_SIZE / 2 + NODE_SPACE_X, NODE_SPACE_Y * 3, "魔力アップ I", h_icon, 10, "魔法攻撃力が少し上昇します。", false });

    // ティア 3 (ID 1から派生)
    // ★★★ 修正箇所: (int)キャストを追加 ★★★
    nodes_.push_back(SkillNode{ 3, 1, VIEWPORT_W / 2 - NODE_SIZE / 2 - (int)(NODE_SPACE_X * 1.5), NODE_SPACE_Y * 5, "攻撃力アップ II", h_icon, 30, "物理攻撃力が中程度上昇します。", false });
    nodes_.push_back(SkillNode{ 4, 1, VIEWPORT_W / 2 - NODE_SIZE / 2 - (int)(NODE_SPACE_X * 0.5), NODE_SPACE_Y * 5, "HPアップ I", h_icon, 20, "最大HPが少し上昇します。", false });

    // ティア 3 (ID 2から派生)
    // ★★★ 修正箇所: (int)キャストを追加 ★★★
    nodes_.push_back(SkillNode{ 5, 2, VIEWPORT_W / 2 - NODE_SIZE / 2 + (int)(NODE_SPACE_X * 0.5), NODE_SPACE_Y * 5, "魔力アップ II", h_icon, 30, "魔法攻撃力が中程度上昇します。", false });
    nodes_.push_back(SkillNode{ 6, 2, VIEWPORT_W / 2 - NODE_SIZE / 2 + (int)(NODE_SPACE_X * 1.5), NODE_SPACE_Y * 5, "MPアップ I", h_icon, 20, "最大MPが少し上昇します。", false });

    // ティア 4 (深い位置のノード)
    // ★★★ 修正箇所: (int)キャストを追加 (不要でしたが安全のため残します) ★★★
    nodes_.push_back(SkillNode{ 7, 3, VIEWPORT_W / 2 - NODE_SIZE / 2 - (int)(NODE_SPACE_X * 1.5), NODE_SPACE_Y * 7, "究極の一撃", h_icon, 100, "強力な一撃を繰り出す究極スキル。解放コストが非常に高い。", false });
    nodes_.push_back(SkillNode{ 8, 7, VIEWPORT_W / 2 - NODE_SIZE / 2 - (int)(NODE_SPACE_X * 1.5), NODE_SPACE_Y * 9, "マスタリー", h_icon, 200, "すべての能力を向上させる。ツリーの最終ノード。", false });
...
bool SkillTree::is_parent_unlocked(int node_id) const {
    // node_idがnodes_のインデックスとして機能している前提 (0から始まる連番)
...
void SkillTree::handle_input() {
    ...
    // --- 2. クリック処理 (ドラッグ中でないとき) ---
    // (GetMouseInput() & MOUSE_INPUT_LEFT) のチェックを外すことで、ボタンが離された瞬間にクリックと判定する
    if (!is_dragging_ && (mouse_button & MOUSE_INPUT_LEFT)) {
        ...
        if (selected_node_id_ != -1) {
            ...
            int btn_y = VIEWPORT_H - 10 - 50; // パネルのY座標から計算
